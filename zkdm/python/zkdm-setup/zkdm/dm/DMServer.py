...
class RegHost(threading.Thread):
	def __init__(self):
		...
		self.myip = zkutils().myip_real()
		self.mymac = zkutils().mymac()
		self.host_type = None
		self.sip = None
		self.sport = None
		self.listByMacUrl = None
		try:
			f = io.open(r'../host/config.json', 'r', encoding='utf-8')
			s = json.load(f)
			self.host_type = s['host']['type']
			self.sip = s['regHbService']['sip']
			self.sport = s['regHbService']['sport']
			f.close()
		except:
			rc['info'] = 'can\'t get host info'
			rc['result'] = 'err'
		self.listByMacUrl = r'http://%s:%s/deviceService/listByMac?Mac=%s'%(self.sip, self.sport, self.mymac)
