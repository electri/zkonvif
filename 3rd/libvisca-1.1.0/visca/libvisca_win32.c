...
{
	DWORD iBytesWritten;
	WriteFile(iface->port_fd, packet->bytes, packet->length, &iBytesWritten,
		  NULL);
...
uint32_t _VISCA_get_packet(VISCAInterface_t * iface)
{
	...
	while (iface->ibuf[pos] != VISCA_TERMINATOR) {
		...
		rc = ReadFile(iface->port_fd, iface->ibuf + pos, 1, &iBytesRead,
			      NULL);
		if (!rc || iBytesRead == 0) {
			// Obtain the error code
			//m_lLastError = ::GetLastError();
...
uint32_t VISCA_open_serial(VISCAInterface_t * iface, const char *device_name)
{
	...
	m_hCom = CreateFile(device_name, GENERIC_READ | GENERIC_WRITE, 0,	// exclusive access
			    NULL,	// no security
			    OPEN_EXISTING, 0,	// no overlapped I/O
			    NULL);	// null template
