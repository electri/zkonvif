...
public:
	...
	ts_list_head() :linkmutex(), list_o_items() {
	}
...
void
ts_list_head::RemoveListItem(ts_list_item * li)
{
	bool is_empty;
...
class ChatterSession : public virtual SocketPort, public virtual ts_list_item
{
public:
	...
	// When you create a ChatterSession it waits to accept a// connection.  This is done by it's own
	ChatterSession(TCPSocket & server, SocketService * svc, ts_list_head * head) 
		  ...
		  ts_list_item(head) 
	{
		...
		cerr << "connecting from " << ia.
			getHostname() << ":" << port << endl;
	...
	virtual void pending() {
		...
		try {
			...
			while ((len = receive(buf, sizeof(buf))) > 0) {
				...
				for (ts_list::iterator iter = listhead->list_o_items.begin(); iter != listhead->list_o_items.end();	iter++) {
					ChatterSession *sess = dynamic_cast <ChatterSession *>(*iter);
					if (sess != this) {
						sess->send(buf, len);
						sent = true;
					}
				...
				if (!sent) {
					send((void *)"No one else listening\n",
...
public:
	...
	void run() {
		while (1) {
			try {
				...
				new ChatterSession(*(TCPSocket *) this, exec->service, exec);
			}
			catch (...) {
	...
	ChatterThread(InetHostAddress &machine, int port, CCExec * inexec)
		: TCPSocket(machine, port), Thread(), exec(inexec) 
	{
		start();
	}
...
CCExec::RunApp(char *hn)
{
	...
	// port as well.
	try {
		my_Chatter = new ChatterThread(machine, 3999, this);
	}
	catch (...) {
		cerr << "Failed to bind\n";
		return false;
